...
class LLMLambda:
    ...
    def invoke(self, config):
        ...
        config.print_logs(f"Query: {self.context}")
        result, _ = self.model.single_query_response(
            context, 
            structure=self.structure, 
            temperature=self.temperature
        )
        result = result.content if self.structure is None else result.parsed
        config.set_by_name(self.output_name, result)
...
class Planner:
    def __init__(
        ...
        self.self_consistency_rounds = 1 if self_consistency_rounds is None else self_consistency_rounds
    ...
    def self_consistency_choice(self, config, plans):
        ...
        result, _ = self.model.single_query_response(planner_cot_voter_prompt, structure=SelfConsistencyFormat)
    ...
    def cot_planner(self, config):
        ...
        result, _, probs = self.model.single_query_response(planner_prompt, structure=PlanFormat, need_logprobs=True)
        result = result.parsed 
        config.probs.append(probs)
        config.print_logs(f"Response: \n{result}")
        return "Empty" if len(result.plan) == 0 else "\n".join([f"{n}: {el}" for n, el in enumerate(result.plan)])
    ...
    def tot_planner(self, config):
        ...
        while current_step < self.tot_rounds:
            current_plan_str = "Empty" if len(current_plan) == 0 else "\n".join([f"{n}: {el}" for n, el in enumerate(current_plan)])
            planner_prompt = \
            f"""
            {self.context}
            You can do these and only these actions as steps:
            ...
            {current_plan_str}
            """  
            result, _ = self.model.single_query_response(planner_prompt, structure=PlanStepFormat)
            result = result.parsed
            ...
            {options_str}
            """
            result, _ = self.model.single_query_response(planner_tot_discriminator_prompt, structure=PlanChoiceFormat)
            result = result.parsed
            config.print_logs(f"Response: {result}")
            next_option = options[result.chosen_option_index]
            current_plan.append(next_option)
            current_step += 1
            if result.is_terminating == True:
                break
        return "Empty" if len(current_plan) == 0 else "\n".join([f"{n}: {el}" for n, el in enumerate(current_plan)])    
...
class PlannerDomainReflector:
    ...
    def invoke(self, config):
        ...
        for i in range(self.max_rounds):
            ...
            {current_plan}
            """
            result, _ = self.model.single_query_response(planner_critic_cells_prompt, structure=PlanReflector)
            result = result.parsed
            config.print_logs(f"Response: {result}")
...
class ToolsExecutor:
    def __init__(
        ...
        self.call_tools = {model.get_tool_from_pydantic(tool)['function']['name']:call for tool, call in tools}
    ...
    def invoke(self, config):
        ...
        while True:
            message, usage = self.model.single_query_response(dialog, tools=self.llm_tools)
        ...
        for node in graph:
            if isinstance(node, tuple):
                if len(node) == 2:
                    ...
                    if right not in builder.nodes and right != END:
                        builder.add_node(right, lambda _, node=node[1]: node.invoke(self.config))
                    ...
                    if right_true not in builder.nodes and right_true != END:
                        builder.add_node(right_true, lambda _, node=node[2]: node.invoke(self.config))
                    if right_false not in builder.nodes and right_false != END:
                        builder.add_node(right_false, lambda _, node=node[3]: node.invoke(self.config))
            ...
            if isinstance(node, list):
                for l, r in zip(node, node[1:]):
                    ...
                    if right not in builder.nodes and right != END:
                        builder.add_node(right, lambda _, node=r: node.invoke(self.config))
                    builder.add_edge(left, right)
